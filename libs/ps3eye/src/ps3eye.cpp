...
#define UVC_STREAM_FID	(1 << 0)
    ...
    public:
        ...
        {
			// libusb_free_transfer already called, so xfr is not allocated anymore! not sure why we need this stuff
			/*
    ...
    void PS3EYECam::ov534_reg_write(uint16_t reg, uint8_t val)
    {
		...
		if(usb_buf != NULL) { // my fixes when using PS3 with generic USB camera
			usb_buf[0] = val;
